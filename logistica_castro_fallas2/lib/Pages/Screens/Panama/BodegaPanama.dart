...
class BodegaPanama extends StatefulWidget {
  const BodegaPanama({Key? key}) : super(key: key);
  Widget verticalSpace({double? space}) => SizedBox(height: space ?? 10.0);
  @override
  _FormState createState() => _FormState();
}

class _FormState extends State<BodegaPanama> {
  List<dynamic>? dataA;
  List<dynamic>? dataS;
  String ship = '';
  ...
  Future fetchDataA() async {
    ...
    response = await http.get(Uri.parse(
        'https://dynamicscf.azurewebsites.net/dynamics/WHSPanamaBodega?name=$codigo'));
