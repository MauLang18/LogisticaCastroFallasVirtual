...
class ActivosC extends StatefulWidget {
  const ActivosC({Key? key}) : super(key: key);
  Widget verticalSpace({double? space}) => SizedBox(height: space ?? 10.0);
  @override
  _FormState createState() => _FormState();
}

class _FormState extends State<ActivosC> {
  List<dynamic>? dataA;
  List<dynamic>? dataS;
  String ship = '';
  ...
  Future fetchDataA() async {
    ...
    response = await http.get(Uri.parse(
        'https://dynamicscf.azurewebsites.net/dynamics/listarCertificadosA?name=$codigo'));
