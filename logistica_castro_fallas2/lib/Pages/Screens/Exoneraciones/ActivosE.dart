...
class ActivosE extends StatefulWidget {
  const ActivosE({Key? key}) : super(key: key);
  Widget verticalSpace({double? space}) => SizedBox(height: space ?? 10.0);
  @override
  _FormState createState() => _FormState();
}

class _FormState extends State<ActivosE> {
  List<dynamic>? dataA;
  List<dynamic>? dataS;
  String ship = '';
  ...
  Future fetchDataA() async {
    ...
    response = await http.get(Uri.parse(
        'https://dynamicscf.azurewebsites.net/dynamics/listarExoneracionesA?name=$codigo'));
